...
class FaceDataset(data.Dataset):
    ...
    def __init__(self, root):
        ...
        self.root = root
        df_path = pd.read_csv(root + '\\data.csv', header=None, usecols=[0])
        df_label = pd.read_csv(root + '\\label.csv', header=None, usecols=[1])
...
def main():
    # 数据集实例化(创建数据集)
    train_dataset = FaceDataset(root='C:\\Users\\yangdong\\PycharmProjects\\pythonProject\\MachineLearning\\Lihongyi')
    val_dataset = FaceDataset(root='C:\\Users\\yangdong\\PycharmProjects\\pythonProject\\MachineLearning\\Lihongyi')
    # 超参数可自行指定
